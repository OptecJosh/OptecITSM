...
- **Web chat channel** *(setup in place; live widget in progress)*: An embeddable website chat widget so visitors can raise tickets from any site, alongside email and WhatsApp. A widget is the self-hosted twin of a WhatsApp number — it drives one `messaging_channels` row (`channel_type='webchat'`, `provider='freeitsm'`, no external provider or credentials) so an inbound message flows through the same ticket/inbox/reply pipeline as every other channel. Managed under **Tickets → Settings → Web chat** (its own `tickets.webchat` capability, separate from the WhatsApp credentials tab): create a widget with a greeting, accent colour and launcher label, pin its conversations to a company, and list the site origins allowed to embed it. Saving mints a **public widget key** and a copy-paste `<script>` **embed snippet**. The key is not a secret (it ships in the site's page source); abuse is contained by the per-widget **origin allowlist** and rate limiting. Config stored in `webchat_widgets` (keyed 1:1 to its channel row); helpers in `includes/webchat/`, config API under `api/webchat/`. The browser chat window itself and the pre-chat name/email prompt are the next build phase.
