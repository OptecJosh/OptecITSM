...
> **Status: Stages 1 & 2 — engine foundation + visual canvas builder.** Schema (`workflows` + `workflow_executions`), engine class (`workflow/includes/engine.php`) with trigger/operator/action catalogues + `dispatch(event, payload)` + `manualFire(workflowId, payload)`, **Process Mapper-style canvas editor** (dot-grid, snap-to-grid, draggable nodes for trigger / condition / action with distinctive shapes — pill / diamond / rounded rectangle, auto-routed SVG connectors), slide-in detail panel that swaps body based on selection (workflow / trigger / condition / action), list landing page with active/inactive status pills, "Test fire" button for synthetic-payload runs, recent-executions panel. v1 action handler is `log_message` so the engine is exercisable end-to-end without host modules wired up yet. AI co-author + real action handlers (set ticket status, send email, create task, Graph API actions) + dry-run + Watchtower integration + starter recipes all planned for subsequent commits.
