...
| **Workflows** | `workflow/` | Amber `#f59e0b` | Cross-module automation engine — triggers, conditions, actions. Visual canvas builder, AI co-author, starter templates, dry run, outbound webhooks |
...
### Timezone (per-analyst display zone — `includes/timezone.php`)
Every datetime is **stored in UTC** (`UTC_TIMESTAMP()` on write) and shown in each analyst's chosen **display timezone**, selectable in `System &rarr; Preferences` (persists as the `timezone` key in `user_preferences`). There is no global timezone setting &mdash; a single zone can't serve a team spread across countries. **Resolution** (in priority order): logged-in analyst's `timezone` preference → server default (`date_default_timezone_get()`, i.e. the value in `config.php`). `config.php` remains the fallback for any no-user context (cron, SLA engine, email polling) and the default until an analyst picks one. A page calls `Tz::init()` after the session is available, emits `Tz::scriptTag()` in `<head>` (publishes `window.USER_TIMEZONE`), and converts UTC → the analyst's zone explicitly via `fmt_local()` (PHP) or, for JS-rendered dates, by passing `window.USER_TIMEZONE` into the `Intl`/`toLocale*` `timeZone` option. Deliberately side-effect-free (no global `date_default_timezone_set()` override), so server-side date math stays on UTC. Rolled out across **all modules** (shared `assets/js/tz.js` is loaded on ~130 pages). Display branches on **three kinds of stored date**: (1) **server-stamped UTC** timestamps (created/updated/received/audit/comments/webhook-deliveries) → converted to the analyst zone; (2) **user-entered naive wall-clock** scheduling values from `datetime-local` (change/outage windows, PIR, ticket work-start, calendar events) → shown **as typed**, never converted (a "2pm" window reads 2pm for everyone; true zone-aware scheduling is a future phase); (3) **date-only** values (expiry/renewal/due dates, CI date properties) → never converted (would shift the day). Storage nuances respected: the `calendar` module stores naive-local by design, and `system-wiki` (SQL Server, `GETDATE()` local) is left unconverted. See `docs/design/timezone-per-user.md` and the **Timezones and Time Handling** wiki page.
...
- **Ticket-to-ticket links**: Link tickets to each other as **Related**, **Duplicate of**, or **Parent/Child** (right-click a ticket or use the reading-pane *Link to ticket* button, pick a relationship, search the target). The reading pane shows a **Linked tickets** strip grouped by relationship, each badge jumping to that ticket with an ✕ to unlink; directional links read correctly from both sides ("Parent of" ⟷ "Child of"). A ticket has at most one parent (a parent has many children) and at most one duplicate-master; self-links and parent/child loops are blocked, and every link/unlink is audited on both tickets. v1 is **informational** (links display + audit, no automatic state changes). On multi-company installs links are **same-company only** (the picker is scoped to the source ticket's company). Stored in a self-referential `ticket_links` table (`source_ticket_id`, `target_ticket_id`, `relation_type`)
...
- **Web chat channel**: An embeddable website chat widget so visitors can raise tickets from any site, alongside email and WhatsApp. Managed under **Tickets → Settings → Web chat** (its own `tickets.webchat` capability, separate from the WhatsApp credentials tab): create a widget with a greeting, accent colour and launcher label, pin its conversations to a company, and list the site origins allowed to embed it. Saving mints a **public widget key** and a copy-paste `<script>` **embed snippet** you drop onto any site. That snippet loads a self-contained widget (`api/webchat/widget.js`) — a floating launcher + chat panel rendered inside a **Shadow DOM** so the host site's CSS can't touch it. A visitor gives their name + email (the pre-chat gate), types a message, and it's ingested onto the shared ticket membrane as a **Web chat**-origin ticket in the pinned company; analysts reply from the normal reading-pane composer and replies reach the visitor by polling. **Trust model:** the widget key is *not* a secret (it ships in the site's page source), so it can only *start* a conversation, and only from an allowlisted origin (checked via `Origin`, falling back to `Referer` for same-origin embeds). Reading or posting to an existing conversation requires its **per-conversation token**, held only in that one visitor's browser — so a leaked key never exposes anyone's chat. Basic per-IP / per-conversation rate limiting on top. Web chat is a self-hosted channel — one `messaging_channels` row (`channel_type='webchat'`, `provider='freeitsm'`, no external provider or credentials, a no-op `FreeitsmProvider` for the shared reply path) — so it reuses the existing ticket/inbox/reply pipeline wholesale. Schema: `webchat_widgets` (1:1 to its channel), `webchat_conversations` (the browser-token → ticket map) and `webchat_messages` (the pre-ticket transcript); helpers in `includes/webchat/`, config + public API under `api/webchat/`. **Per-widget controls:** point a widget at a **business-hours calendar** so it presents itself as "leave a message" out of hours (the enquiry still becomes a ticket, with your offline message shown), and — with a Knowledge AI provider configured — turn on **AI answers** that reply to visitors grounded strictly in your published Knowledge articles (it never invents answers). AI runs in one of two modes: **assist** raises the ticket immediately and posts the AI's replies alongside it for the analyst; **deflect** holds off, letting the visitor and AI chat privately with *no ticket raised* until the visitor escalates. **Escalation** offers up to two buttons — *Talk to a person* (promotes the chat to a live ticket, transcript as the opening message) and *Email me back* (opens a ticket with an AI-written summary as the body and the full chat log attached as a `.txt`); analyst replies mirror back into the AI chat window. *Roadmap: sending the offline / email-me-back reply out from the company's service-desk mailbox.*
...
- **Settings**: Departments, ticket types, origins, mailboxes (Microsoft 365, Google Workspace, or basic IMAP/SMTP), messaging channels (WhatsApp), web chat widgets, email templates, analysts, teams
...
- **Help** (`system/help/`): In-app, step-by-step guides for **every** System area — a searchable landing grid plus a dedicated page per area, including a flagship **Single Sign-On** guide that walks through both single-company and multi-company (MSP) setup. The Companies page links to the SSO guide via a banner. Pages share one layout (`_init`/`_top`/`_bottom`) with left-nav + scroll-spy.
  - **Help registry** (`system/help/_registry.php`): one entry per guide, owning its hero, standfirst, section list and search synonyms. It drives three things at once, so they cannot drift apart: the card on the landing grid, the left-hand nav on the guide itself (a page never restates its own sections — it just names its slug), and the search index. Cards are produced by joining this registry to `system/includes/areas.php`, which keeps titles/descriptions/icons translated and, crucially, means **an area with no guide yet gets no card** rather than a card linking to a page that doesn't exist — the gap is listed as "not yet documented" at the foot of the grid instead.
  - **Search**: filters the guides live by title, description, keyword synonyms, standfirst and section headings — indexed by what you'd *call* the problem ("locked out", "can't see tickets", "not sending", "wrong company") as well as by feature name. A match on a section heading deep-links straight to that section of the guide. Press `/` to jump to the box; `?q=` prefills it, so a search result can be linked to.
...
- **Analysts** (`system/analysts/`): Manage analyst accounts — create/edit/deactivate users, reset passwords, assign a single sign-on provider, set per-analyst team membership, and (on multi-company installs) grant access to specific companies. Promoted here from Tickets → Settings because analysts are the app's users and work across every module; the endpoints still live under `api/tickets/`, so the move is UI-only. Company access controls are shown only when more than one company exists (invisible at N=1). The company chip and edit dialog show **effective** access — the companies an analyst can actually reach, whether granted directly or **inherited from a team** (team-granted companies are shown read-only here, with a note pointing to System → Teams).
- **Teams** (`system/teams/`): Manage the teams analysts belong to — create/edit/deactivate teams, and configure a team fully from one screen via per-row **Manage departments**, **Manage members** and (multi-company installs) **Manage companies** pickers, with live department/analyst counts and a company-access chip. Teams are a shared entity consumed by Tickets, Tasks, Contracts and the Workflow engine (assignment + access), so they live at the install level rather than inside one module's settings. **Company access via a team is additive**: a team can grant its members access to specific companies (or all companies), unioned with each member's own access — so *"the Acme team handles Acme + Globex"* is one grant instead of ticking companies on every analyst. Team grants **default to nothing** (unlike the analyst all-access default) so existing teams never silently widen access on upgrade; team company access is enforced through the single `getAccessibleTenantIds()` choke-point in `includes/tenancy.php`, so it applies everywhere (tickets, problems, changes, the service layer). Teams themselves stay global (no per-company scoping — a team isn't *owned* by a company, it just carries company grants). The department↔team and analyst↔team links are **many-to-many, editable from either side** — here on the team side, or from the analyst side on **System → Analysts** and the department side on **Tickets → Settings → Departments** (departments are a ticket concept and stay there); every side writes the same join tables (`department_teams` / `analyst_teams`, plus `team_tenant_access` for company grants).
- **Roles** (`system/roles/`): Grant a non-administrator the right to manage **one specific module's settings** without making them a full System administrator — RBAC "Layer 2", the fine control that sits on top of module access. A role bundles *capabilities*, assigned to analysts and/or whole teams. Permissions go down to the individual **settings tab**, so you can let someone own the ticket statuses and SLAs while keeping them out of the mailbox credentials — with a per-module "manage everything here" umbrella for the common case, and a **sensitive** badge on any capability that reaches credentials, email or money. Deny by default; System administrators bypass the layer entirely. Design doc: `docs/design/rbac.md`.
  - Capabilities are declared as constants in `includes/capabilities.php` (`Cap::LMS_MANAGE`) and **must always be passed as constants, never as strings** — a mistyped string fails *closed*, producing a permanent 403 that looks exactly like working software and is invisible to administrators, who bypass the check. A mistyped constant is an immediate error. The registry and the Roles picker are generated from those constants, so the two can't drift.
  - Settings that live in the shared `system_settings` table are owned per key in `includes/settings_keys.php`, because the tabs that write them share one generic endpoint. **A key no module claims cannot be written through it.**
  - A module declares its settings tabs in `<module>/settings/manifest.php`, and the tab bar is **rendered from that manifest** — so a tab you lack the capability for is never emitted into the page, rather than hidden with CSS. `capSelfCheck()` and `settingsManifestSelfCheck()` prove the manifest, the capability registry and the setting-key map agree.
  - **Converted so far:** 16 of 17 modules and 76 capabilities. Every module except **System**, which is administrator-only by design.
  - **Two layers, composed.** Module access (below) decides which modules you can *enter*; a role decides whether, once in, you can also *administer* its settings. A role is a named bundle of **settings capabilities** (keys like `lms.manage`) assigned to analysts and/or whole teams — resolved at one choke-point (`getAnalystCapabilities()` in `includes/rbac.php`) as the union of directly-assigned and team-inherited roles, exactly the shape module and company access already use.
  - **Deny by default; `is_admin` bypasses.** With no role, a non-admin has no settings access anywhere — the correct default for an administrative surface. System administrators implicitly hold every capability, which is what makes deny-by-default safe on upgrade: the instance owner keeps everything and the tightening only ever bites non-admins. The capability **registry lives in code** (`rbacCapabilities()`), so the DB can never hold a capability the app doesn't define, and retiring one in code retires it everywhere with no data migration.
  - **Enforced server-side**, page *and* API, failing closed (`requireCapability()` / `requireCapabilityJson()`) — not just hidden, so a settings URL or endpoint can't be reached without the capability. Managing roles is itself administrator-only. New tables `rbac_roles` / `rbac_role_capabilities` / `rbac_analyst_roles` / `rbac_team_roles`. The **LMS is the pilot** (`lms.manage`); other modules' settings move behind their own capability over time.
- **Module Access** (`system/modules/`): Control which modules each analyst can use — granted to **individual analysts and to teams**, resolved together under a configurable policy, and **enforced server-side** (GitHub #30; mirrors the company-access model)
  - **Sources combine at one choke-point** (`getAnalystAllowedModules()`): an analyst's access is their own grant plus every team they belong to. New `can_access_all_modules` flag on analysts (default on) and teams (default off), plus a `team_modules` table mirroring `analyst_modules`. The all-access toggle appears on the Analyst and Team forms — and, via the **Access level** panel, directly on this screen (a filterable all-modules toggle per team/analyst) so you can grant/revoke it without leaving the page
  - **Most/least-permissive policy** (`module_permission_mode`, default **most**): under **most** any source that grants a module wins (union); under **least** the analyst needs it from their own access *and every team they're in* (intersection) — a deliberately strict model where an empty team locks its members out, so the screen shows a **persistent explainer** whenever least is active
  - **Summary screen**: modules as rows, with pills for the teams and analysts that can reach each one, a per-module edit modal, the mode toggle, and an **effective-access tool** — pick an analyst and see every module with ✅/❌ and the reason why (granted directly / via a team / blocked by strict mode)
  - **Enforced server-side, not just hidden**: a page guard (`requireModuleAccess`) bounces a denied analyst off any module's pages, and a JSON guard (`requireModuleAccessJson`) returns `403` on ~230 write/mutation endpoints across all 20 modules — so a crafted URL or POST can't bypass the hidden launcher. Shared read endpoints other modules or the login page depend on are deliberately left open, as with the admin gate
  - The **System module is separate** — gated by `is_admin`, not by module access — so an administrator can always reach System → Modules to fix a misconfiguration
  - **Safe upgrade**: existing analysts are grandfathered (unrestricted stay all-access; previously-restricted keep their rows), teams start with no grants, and the default **most** mode means nobody's effective access changes on upgrade
...
**Roles: learner vs manager** (the RBAC pilot — first module wired to `docs/design/rbac.md`)
- The LMS is the first module split by an RBAC "Layer 2" capability, **`lms.manage`**. Module access lets you *take* assigned training; the capability lets you *administer* — author, upload, run groups, assign, and see everyone's progress. System administrators hold it implicitly (bypass).
- **My Courses** (`my-courses.php`): the learner's landing — only the courses assigned to them (via their groups), with their own status and a Start/Resume/Review button. Everyone with the LMS module gets this; it shows no management and no other people's data. `lms/` routes non-managers here automatically, and the header nav adapts (learners see My Courses + Help; managers additionally get Dashboard + Settings).
- **Enforced server-side, everywhere** (`includes/lms_access.php`): the management dashboard/editor and every management API sit behind `lms.manage`; playback and the learner content/progress APIs are gated to **assigned-to-me-or-manager**, so a learner can neither reach the admin screens nor open (or pull the content/answer-key of) a course that wasn't assigned to them — by URL or by direct API call. Managers may open any course, which is how Preview works.
...
- **The trigger catalogue spans every module (138 triggers and counting).** Rich domain events fire from their host modules — e.g. `ticket.created` (`api/tickets/create_ticket.php`), `ticket.status_changed` / `ticket.priority_changed` / `ticket.assigned` / `ticket.deleted` / `ticket.restored`, `form.submitted` (payload includes a `submission.fields` map of every answer keyed by label, plus the first email-type answer as `submission.email`), `task.completed` (only on the transition *into* a closed status) / `task.deleted`, `change.approved` (from both a CAB vote crossing the approval threshold and a manual status edit into Approved) / `change.deleted`, `problem.created` / `.status_changed` / `.deleted`, `cmdb.object.created` / `.updated` / `.deleted`, `knowledge.published` / `.updated` / `.archived`, `service_status.incident_created` / `.incident_updated` / `.incident_resolved` / `.incident_deleted`, `morning_check.recorded`, and `software.application_discovered` (fired from all three agent-ingest paths when a new app first appears). On top of these, **every reusable entity and settings lookup** gets explicit `.created` / `.updated` / `.deleted` triggers — tickets/assets/change/problem/task/CMDB/contracts settings, calendar events + categories, suppliers + contacts, software licences, network diagrams, and more — generated from a `crudEntities()` map so the list stays maintainable while every entity+action is still its own explicit trigger (no generic catch-all, and **no dead triggers** — only entities actually wired to fire are registered). UI settings endpoints emit via a best-effort `wf_emit()` helper; shared services call the engine directly. Because the list is now dozens deep, the editor's **trigger picker is searchable**. Each dispatch is wrapped in a host-side try/catch on top of the engine's own internal swallowing so a workflow can never break the host module's request.
...
- **Outbound webhooks** (`send_webhook` action): the universal way to push FreeITSM events into any external system — Slack, Teams, Discord, PagerDuty, Zapier/Make, or a home-grown endpoint. Attach it to any trigger and it POSTs a JSON payload to a configured URL. One-click **presets** (Slack / Teams / Discord) wrap a templated `message` in that platform's expected shape; the **Full record** preset sends the entire object as JSON — the *exact same shape the REST API returns* for that resource (it reuses the API serializers), so downstream systems get a rich, typed, consistent payload for free; **Custom** mode sends a raw JSON `body` you template yourself with `{{ticket.subject}}`-style variables (including `{{ticket.full}}`, which embeds that whole API-shaped object), so it can match any target's contract. The full-object enrichment is a registry of loaders keyed by payload object (`WorkflowEngine::fullObjectLoaders()`) and covers every event that carries a record with a clean `GET /{id}` — `ticket`, `change`, `problem`, `task`, `asset`, `article` (knowledge), `contract`, `supplier`, `calendar_event`, `software_licence` and `incident` (service-status) — reusing each resource's own SELECT + serializer so `.full` is byte-identical to the API (it runs the raw select, *not* the `apiLoadX()` helpers, since those `apiError()`/exit on a missing row and would abort the host request). Loaded lazily (only when the Full-record preset is chosen or the body references `.full`) and best-effort (a load failure just omits `.full`). CMDB objects and network diagrams are deferred — their GET hydrates deep child collections beyond a single fetch — so those events carry id+name inline. An optional signing **secret** HMAC-SHA256-signs the payload (`X-FreeITSM-Signature`) so the receiver can verify it. A **Send test** button in the workflow editor fires the webhook at your URL right there — with sample ticket data — and shows the endpoint's real response before you save, so you never have to guess whether it's wired up correctly. **Delivery is asynchronous**: the action enqueues to `webhook_deliveries` and returns instantly, and a background worker (`cron/webhook_deliveries.php`) delivers each with exponential-backoff retries and dead-lettering — so a slow or dead endpoint never blocks the run. **System → Webhooks** (also linked from **Workflows → Webhook deliveries**) is the single home for delivery: at the top a live check of whether the background worker is running (with the exact command to schedule it if not), then an **Overview dashboard** — 7-day success-rate / volume / average delivery-time / queued / dead-letter KPIs, a 14-day delivered-vs-failed volume chart, and tables of the top target endpoints and source workflows each with their own success rate (all computed server-side from `webhook_deliveries` on load) — then the full delivery log, which logs every send (status, attempts, the full request payload and the full response, last error) with a Replay button and payload viewer. Because it's just a workflow action, it inherits conditions, templating and the execution audit for free — and it's the outbound half that makes FreeITSM a first-class integration hub (the inbound half being the REST API, email and messaging ingest). Cron setup: `docs/webhook-cron-setup.md`.
- **Time-based triggers** (`sla.warning`, `sla.breached`, `contract.expiring`, `asset.warranty_expiring`): every other trigger hangs off a **write path** — someone saved a ticket, so there's a moment to `dispatch()` from. These aren't events at all: *nothing happened, time passed*. So a scheduled job has to go looking (`cron/workflow_scheduled.php` for the expiries; the existing SLA breach-check cron emits the SLA pair, since it already walks every open ticket and computes its state — free, and it can't drift from what the SLA emails believe). The SLA events fire **whether or not** an email notification rule matches: rules decide who gets an *email* and have nothing to say about automation. Expiries fire at **90/30/7/1 days** with `window_days` in the payload, so *"only remind me at 30 days"* is a condition, not something the workflow must schedule itself. **The hard part is not firing twice.** A time-based condition *stays true* — a breached SLA is still breached five minutes later — so naive detection would re-escalate forever. `workflow_scheduled_emissions` is a ledger with a **UNIQUE key + `INSERT IGNORE`**: only the insert that creates a row dispatches, making the *database* the arbiter, so overlapping cron runs can't double-fire. Each row carries a **fingerprint** — the state it fired against (SLA target minutes, contract end date) — because plain fire-once is wrong: **raise a ticket's priority and its SLA target shrinks**, and the new tighter deadline must be allowed to escalate again; renew a contract and next year's reminder must still fire. And **nothing is recorded when no active workflow is listening**, or the ledger would say "already fired" for every contract currently in-window and switching a workflow on tomorrow would leave it invisibly silent. ⚠️ **If the crons aren't scheduled these triggers never fire** — `docs/workflow-scheduled-cron-setup.md`.
- **Starter templates** (`workflow/includes/templates.php`, **New from template** on the Workflows list): a gallery of ready-made recipes for the jobs most service desks automate first — auto-acknowledge a new ticket, thank the requester on close, P1 incident response, alert on escalation to P1, broadcast a status-page incident, **new-starter form → IT + HR + Facilities tickets + a chase-it task**, raise an implementation task when a change is approved, announce a published knowledge article. The design problem a recipe library has to solve is that action args like `status_id` are **numeric ids that differ per install**, so a template can never hardcode `status_id: 5`. Instead a template declares lookups *by name* — `['$lookup' => 'ticket_status', 'names' => ['Closed', 'Resolved', …]]` — and the resolver maps the first matching name to this install's real id as the recipe is cloned (candidate order matters: a desk with both *Critical* and *High* correctly gets Critical for P1). Values only the user can supply (a Slack webhook URL) use a `['$configure' => '…']` marker. Anything that can't be resolved is cloned **blank** and reported in an `unresolved` list which the editor renders as a "these fields still need you" banner — a recipe always clones, it just tells you honestly what it couldn't fill in. Cloned workflows are **always inactive**, so nothing a recipe contains can fire until it's been read and switched on. Every template's trigger and action types are real, wired ones — a recipe that couldn't actually run would be worse than no recipe.
- **Dry run** (toolbar button beside Test fire): evaluates the conditions for real, then *describes* each action instead of executing it. The run is logged like any other but flagged `is_dry_run`, and each step records `would_run` (the action's catalogue label) plus `would_args` — **the args with `{{variables}}` already substituted**, so you read the actual email subject, the actual chat message, the actual note. Nothing is written, emailed, queued or sent (no ticket mutations, no notes, no `webhook_deliveries` rows) and the workflow's `run_count` / `last_run_datetime` are untouched — a dry run is not a run. Any signing `secret` is redacted rather than echoed back, so the debugging surface can't be used to read a secret out of a saved workflow. Safe to point at a live workflow, and the sensible first thing to do with a freshly cloned template. Contrast with **Test fire**, which genuinely executes (and will really send that email).
- **Merge codes, scoped to the trigger** (`WorkflowEngine::availableVariables($trigger)`): every free-text action arg has an **Edit with variables** button opening a roomy editor with a searchable variable list beside it — click to insert at the cursor, with a live preview that swaps each code for what it stands for. The list is **derived from the trigger**, and that's the load-bearing part: `renderTemplate()` resolves an unknown path to an **empty string**, it doesn't error. So a `knowledge.published` workflow whose webhook body says `{{ticket.subject}}` wouldn't fail — it would silently POST a blank. There is no ticket in a knowledge event, so the picker never offers ticket codes there, and a hand-typed one is flagged under the field ("will come out blank — not available on this trigger"). The catalogue is deliberately distinct from `availableFields()` (which answers "what can I write a *condition* on" — scalars only): it adds `{{event}}` (the trigger name) and `{{<object>.full}}` for objects with a full-record loader, and it advertises open-ended shapes like `{{submission.fields.<your form's label>}}` that can't be enumerated. Labels are *derived* from the path (`ticket.requester_email` → "Ticket · Requester email") rather than hand-mapped, so they can't rot as the 138-trigger catalogue grows.
- **Transport failures are diagnosed, not just reported** (`webhookDiagnoseError()` in `includes/webhook_delivery.php`): a raw cURL error like `SSL certificate problem: unable to get local issuer certificate` tells you what broke, not what to do — and its cause is usually not the webhook at all. A stock **Windows/WAMP install ships PHP with no CA bundle** (`curl.cainfo` and `openssl.cafile` are commented out and no `cacert.pem` is supplied), so the server can't verify *any* HTTPS certificate and every outbound call fails this way. The helper recognises the TLS-trust family (plus DNS / refused / timeout) and returns a plain-English explanation, rendered both in the editor's **Send test** panel and in the **System → Webhooks** delivery log, with a **How to fix this →** deep-link to `workflow/help-ssl.php` — a full guide to what certificate verification is, what a CA bundle is, the exact fix (Mozilla bundle from curl.se; set both settings in **both** php.ini files, Apache's *and* the CLI's, because the delivery worker runs under the CLI — miss it and Send test passes while live deliveries keep failing), and why "just disable SSL verification" is the wrong answer (it keeps the encryption but stops checking *who you're talking to*). Webhooks therefore have **no** verify-SSL toggle, deliberately — unlike the AI settings, which do, for corporate MITM proxies. Diagnosis runs at render time against the stored error, so historic failures get it too, with no schema change.
- **Message formats are data, not code** (`webhook_message_formats` + **Workflows → Settings → Message formats**): the Slack / Teams / Discord presets used to be a PHP `switch`, so adding Google Chat or Mattermost meant a release. But a chat preset *is* just **a JSON body template with a `{{message}}` slot** (`{"text": "{{message}}"}` for Slack, `{"content": "{{message}}"}` for Discord) — meaning the existing **Custom (raw JSON)** mode already did everything a preset does; a preset is a custom body someone named. So they're rows now. Built-ins ship seeded and locked (editing Slack's template in place would break every Slack webhook on the install); **Copy** clones one to edit. Deleting a format still referenced by workflows is refused, naming them. **Escaping is the load-bearing detail:** the template is *decoded*, substitution happens inside string **values**, and `json_encode` does the escaping — we never splice into raw JSON text, so a message containing `"}, {"injected":"yes` can't break the payload or inject structure (verified). The three built-ins remain a **hardcoded fallback** in the engine, so webhooks still send if the table is missing or unreadable; a workflow pointing at a *deleted* format fails loudly rather than posting a mis-shaped body. `custom` and `full` stay in code — they don't wrap a message. Each format also carries a **URL pattern** (so pasting a Discord URL with the format left on Slack warns you instead of producing a mystery 400) and a **markdown hint** shown under the Message box (Discord's `**bold**` vs Slack's `*bold*`).
- **Readable `_name` merge codes.** `{{ticket.priority_id}}` renders `4` — fine for a receiving system, useless in a message a human reads. Every lookup field therefore has a readable twin (`{{ticket.priority_name}}` → "Critical"), derived automatically from the same `FIELD_LOOKUP_TABLES` registry that powers the condition dropdowns, so a new lookup field gets its name code for free. Includes `{{old_priority_name}}` / `{{new_priority_name}}` on the change triggers, so *"escalated from High to Critical"* writes itself. The picker gives the **name** the plain label (*Ticket · Priority*) and marks the numeric one *(id)* — labelling them the other way round would hand the natural-sounding name to the field that renders a meaningless number. Resolved once per run into the payload (`enrichWithLookupNames()`, cached), so the step log, the dry-run preview and every action see the same values; it never clobbers a value the host module set deliberately (`form.submitted` ships a real `form.name`, which wins over anything derived from `form.id`).
- **Webhook credentials encrypted at rest; payloads kept only as long as you choose.** A webhook URL is a bearer credential (anyone holding a Slack/Discord URL can post to that channel) and the signing secret is a true secret — a reader of the database could otherwise forge FreeITSM's signature. Both are **AES-256-GCM encrypted** (reusing `includes/encryption.php`) in `workflows.actions` *and* in `webhook_deliveries.url`, and the URL is **redacted in the delivery log** (`https://discord.com/api/webhooks/…/••••`) so the token isn't sitting in a screen every analyst can open. Encryption is **best-effort by design**: the key is a file an admin creates, and if it's absent FreeITSM stores values as it always did rather than breaking webhooks outright — and says so in a warning on **System → Webhooks** instead of implying a protection it isn't providing. Migration is free (`decryptValue()` passes plaintext through, so pre-encryption rows keep working and encrypt on next save). ⚠️ `webhook_deliveries.url` widened to `VARCHAR(2000)`: encryption inflates a string by ~⅓ + 28 bytes, so a max-length 1000-char URL becomes ~1377 — at the old width MySQL would have silently truncated the ciphertext, which is then **unrecoverable**. Deliberate divergence from the API-key convention: these are **not masked** back to the editor, because a "leave unchanged" placeholder would have to be mapped onto the right action across a save, and the canvas re-sorts actions by Y position — so any positional mapping breaks the moment a node is dragged, silently attaching the wrong secret to the wrong endpoint. Separately, **payload retention** (`webhook_payload_retention_days`, default **7 days**) scrubs `request_body` / `response_snippet` — with the Full-record preset those hold an *entire ticket* in plain text — while **keeping the delivery record** (endpoint, status, timing, errors) for audit until the longer `webhook_delivery_retention_days` removes the row. Trade-off stated in the UI: Replay re-sends the stored body, so a purged delivery can't be replayed, and the endpoint says exactly that rather than POSTing an empty payload to a live endpoint.
...
- **Execution audit**: every run writes to `workflow_executions` with a full step-by-step log, the trigger payload snapshot, start/finish timestamps, status, the dry-run flag and any error.
- **The execution log** (`workflow/executions.php`): every run of every workflow — filter by workflow, status, trigger, date range, dry-run-vs-real, or free text over name/trigger/error; status tallies double as one-click filters; filters live in the URL so a view is bookmarkable, shareable and deep-linkable. Open a run to see **what the engine actually did**: which conditions matched and which didn't *with the actual value it compared against* (so a skipped run finally explains itself — `ticket.subject contains "urgent" | actual "x"`), what each action returned or how it failed, the webhooks the run queued (bridging into the delivery log), and the **trigger payload snapshot** — exactly what the conditions were tested on and what the `{{variables}}` resolved from. Runs stay attributable and searchable after their workflow is deleted, because the name is snapshotted on the row.
- **The Watchtower card**, and *why it has to exist*: the engine **deliberately swallows its own errors** so a broken workflow can never break the ticket save that triggered it. Correct — and it means a workflow can fail on every fire while **nothing anywhere tells you**. The card surfaces failed runs, loop-protection aborts, and dead-lettered webhooks (the workflow "succeeded" — it queued the send — so a message that never arrived would otherwise show up nowhere) over the last 24 hours. It names the failing workflows and shows the error text, because *"3 failures"* sends you hunting while *"Unknown status_id: 999999"* answers it on the spot. Each line deep-links into the pre-filtered execution log. Dry runs are excluded: someone testing isn't an incident.
...
A versioned RESTful API for integrations (monitoring, RMM, scripts, portals). Keys are created under **System → API** with granular per-resource permissions and optional company scoping; authenticate with `Authorization: Bearer fitsm_…`. Front controller (`api/v1/index.php`) + `.htaccess` rewrite gives clean URLs (`/api/v1/tickets/42`); without mod_rewrite the same routes work at `/api/v1/index.php/tickets/42`. Real HTTP status codes; responses are `{data, meta?}` / `{error: {code, message}}`; 60 req/min rate limit per key (overridable per key). Writes behave exactly like the UI: audit rows, template emails, CSAT auto-trigger and workflow-engine events all fire. A machine-readable **OpenAPI 3.0.3 specification** (typed schemas, examples, per-operation errors) is generated live at **`/api/v1/openapi.json`** and **`/api/v1/openapi.yaml`** for Postman, client generators and contract tests — no key required; it validates against the official OpenAPI meta-schema and its schemas are verified against live responses (dev tooling + self-check under `api/v1/dev/` and `api/v1/lib/openapi_check.php`).
