...
- **API** (`system/api/`): Manage keys for the public REST API (`api/v1/`) and explore it interactively
  ...
  - **Granular permissions** per key: a resource × action matrix (tickets read/create/update/delete/restore, notes, conversation, audit, SLA, time entries; assets read/create/update, assignments, history, inventory; requesters, analysts, companies, reference data) — keys start from zero
