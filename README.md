...
- **Execution audit**: every run writes to `workflow_executions` with a full step-by-step log, the trigger payload snapshot, start/finish timestamps, status, the dry-run flag and any error. Future Watchtower integration will surface failed runs as attention cards.
