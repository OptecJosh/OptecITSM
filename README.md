...
- **Break-glass** — local username/password login always remains available as a fallback. With *Allow local login* off the local form is hidden (SSO-only experience) but is never hard-disabled — a "Sign in with a local account" link (and `?local=1`) always reveals it, and the master switch instantly reverts everyone to local login if an IdP is down or misconfigured, so no one can be locked out.
