 ...
 * stored key is decrypted and used — so the user can test the saved
 * config without re-typing the secret.
 *
 * Mirrors api/workflow/test_ai_key.php.
 */
...
try {
    ...
    if (!in_array($provider, FORMS_AI_VALID_PROVIDERS, true)) {
    ...
    // Masked placeholder => fall back to the stored key so the user can
    // test the saved config without re-typing their secret.
    if (isMaskedNoChangeValue($apiKey)) {
        $conn = connectToDatabase();
        $stmt = $conn->prepare("SELECT setting_value FROM system_settings WHERE setting_key = 'forms_ai_api_key'");
        $stmt->execute();
    ...
    $result = $provider === 'anthropic'
        ? formsTestAnthropic($apiKey, $model, $verifySsl)
        : formsTestOpenAI($apiKey, $model, $verifySsl);
...
function formsTestAnthropic(string $apiKey, string $model, bool $verifySsl): array
{
    $body = json_encode([
        'model'      => $model,
        'max_tokens' => 16,
    ...
    $verifyPeer = formsEffectiveSslVerify($verifySsl);
...
function formsTestOpenAI(string $apiKey, string $model, bool $verifySsl): array
{
    $body = json_encode([
        'model'      => $model,
        'max_tokens' => 16,
