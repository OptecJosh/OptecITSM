<?php
/**
 * API Endpoint: Save task status (create or update)
 */
...
try {
    ...
    if ($is_default) {
        $clearSql = "UPDATE task_statuses SET is_default = 0";
    ...
    if ($id) {
        ...
        $sql = "INSERT INTO task_statuses (name, colour, is_closed, is_default, display_order, is_active)
                VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$name, $colour ?: null, $is_closed, $is_default, $display_order, $is_active]);
    }

    $hasDefault = (int) $conn->query("SELECT COUNT(*) FROM task_statuses WHERE is_default = 1")->fetchColumn();
    if ($hasDefault === 0) {
        $conn->exec("UPDATE task_statuses SET is_default = 1 ORDER BY display_order, id LIMIT 1");
    }
